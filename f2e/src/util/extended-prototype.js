...
const extendHTMLElementProtoType = async () => {
    if (!HTMLElement.prototype.collectFormData) {
        HTMLElement.prototype.collectFormData = function (skipWithoutDataFieldInputWarn = false) {
            const result = {};
            const inputs = this.querySelectorAll('input[type="text"],input[type="phone"],input[type="password"],input[type="number"],input[type="hidden"],input[type="date"]');
            inputs.forEach((el) => {
            ...
            selects.forEach((el) => {
                ...
                if (el.hasAttribute('allow-empty') && el.value === '') {
                    ...
                    result[el.dataset.field] = Number(el.value);
                } else if (el.value) {
                    result[el.dataset.field] = el.value;
                }
            });
